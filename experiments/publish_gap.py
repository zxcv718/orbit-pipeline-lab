#!/usr/bin/env python3
"""E3 — 결과를 갱신하는 동안 조회하면 무엇이 보이는가.

두 가지 게시 방식을 같은 조건에서 비교한다.

  (as-is) 통째 교체 : 기존 컬렉션을 비우고 새로 채운다.
                      비우는 순간부터 다 채울 때까지, 조회하는 쪽은 없거나 절반인 결과를 본다.
  (to-be) 버전 교체 : 새 버전 컬렉션에 다 쓴 뒤, 포인터 한 개만 바꾼다.
                      조회하는 쪽은 항상 '완성된 버전'만 본다.

측정: 게시가 진행되는 동안 계속 조회하면서
      - 빈 결과를 본 횟수
      - 불완전한 결과를 본 횟수
      - 그 상태가 이어진 시간
을 센다.

전제: 로컬 MongoDB. 스페이스맵이 결과 저장에 MongoDB를 쓴다는 공개 자료(논문)에 맞춘 것이다.
